...
**If a first action returns no usable content** (empty body, raw metadata, binary, proprietary format), do NOT bounce back to the user with "please paste it" — that's lazy. Try a variant action from the same toolkit via \`composio_search_tools\`. Most platforms have several read paths: download vs export, get-raw vs get-text, get vs read-full, list vs detail. Doc/spreadsheet apps almost always have an "export to text/plain" or "convert to PDF/markdown" action distinct from "download file". Try at least one variant before asking the user to do work the toolkit can do.
